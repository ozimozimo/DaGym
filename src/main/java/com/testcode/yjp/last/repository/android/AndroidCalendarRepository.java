...
//    ArrayList<Calendar> findCalendarByDate(Member member, String start);
    ...
    @Query(value = "select * from calendar_record where calendar_start <= :start and calendar_end >= :end and member_id = :member", nativeQuery = true)
    ArrayList<Calendar> findCalendarByDate(Member member, String start, String end);
